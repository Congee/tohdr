...
//!
//! `IMG_4913.HEIC` 4032x3024, reference = Apple's own base, ours via
//! `tohdr convert --engine apple`. `magick` rows are the same file through
//! libheif:
//!
...
//!
//! Worth knowing anyway: our base's `colr` is `nclx` with primaries
//! *Unspecified*, transfer *Unspecified*, matrix BT.601, and no ICC profile at
//! all. `IMG_4913` is equally Unspecified in its nclx but carries a "Display
//! P3" ICC profile, so a colour-managed decoder can ask. Ours can only be
//! guessed at. The guess is right today on both decoders measured; it is still
//! a declaration this file should be making and isn't.
