...
//!
//! `load_hdr` renders into extended-linear sRGB and clamps negatives away, which
//! is where out-of-gamut colour dies. For each file this renders into both
//! extended-linear sRGB and Display P3, cross-checks them by matrix (to prove
//! CoreGraphics preserves negatives rather than gamut-mapping), then reports the
//! fraction of pixels outside Rec.709/P3/Rec.2020 and the CIEDE2000 cost of the
//! clamp.
//!
//! Read-only: opens sources through ImageIO and writes nothing.
//!
//! Findings and the table they came from: docs/gamut.md.
